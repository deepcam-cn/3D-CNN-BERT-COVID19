...
from torchvision.models.video import r3d_18 as _resnet18

from .BERT.bert import  BERT2


__all__ = ['rgb_I3D64f', 'flow_I3D64f','rgb_resnet50I3D64f', 'rgb_resnet18I3D64f',
...
class rgb_resnet18I3D64f(nn.Module):
    def __init__(self, num_classes , length, modelPath=''):
        super(rgb_resnet18I3D64f, self).__init__()
        self.num_classes=num_classes
        self.dp = nn.Dropout(p=0.8)
        #self.avgpool = nn.AvgPool3d((8, 7, 7), stride=1)
        self.avgpool = nn.AdaptiveAvgPool3d(output_size=(1, 1, 1))            
        self.features=nn.Sequential(*list(_resnet18(pretrained=True).children())[:-2])
        
        self.fc_action1 = nn.Linear(512, 64)
        self.fc_action2 = nn.Linear(64,num_classes)
...
class rgb_I3D64f_bert2(nn.Module):
    ...
    def forward(self, x):
        ...
        #print("classificationOut.shape = {}, sequenceOut.shape = {}".format(classificationOut.shape,sequenceOut.shape))
